 ...
 */
class Simpledownload_Controller_Base_External extends Zikula_AbstractController
{
    /**
    ...
    {
        PageUtil::addVar('stylesheet', ThemeUtil::getModuleStylesheet('Simpledownload'));
    
        $getData = $this->request->query;
        $controllerHelper = new Simpledownload_Util_Controller($this->serviceManager);
    
        $objectType = $getData->filter('objectType', 'download', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'external', 'action' => 'finder');
        if (!in_array($objectType, $controllerHelper->getObjectTypes('controller', $utilArgs))) {
            $objectType = $controllerHelper->getDefaultObjectType('controllerType', $utilArgs);
        }
    
        $this->throwForbiddenUnless(SecurityUtil::checkPermission('Simpledownload:' . ucwords($objectType) . ':', '::', ACCESS_COMMENT), LogUtil::getErrorMsgPermission());
        ...
        $categoryIds = ModUtil::apiFunc('Simpledownload', 'category', 'retrieveCategoriesFromRequest', array('ot' => $objectType, 'source' => 'GET'));
        ...
        $view = Zikula_View::getInstance('Simpledownload', false);
