 ...
 *
 * This is the base repository class for download entities.
 */
class VerySimpleDownload_Entity_Repository_Base_Download extends EntityRepository
{
    /**
     * @var string The default sorting field/expression.
     */
    protected $defaultSortingField = 'downloadTitle';
    ...
    public function getDescriptionFieldName()
   {
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $listHelper = new VerySimpleDownload_Util_ListEntries(ServiceUtil::getManager());
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('download', 'workflowState');
            }
    
            // initialise Imagine preset instances
            $imageHelper = new VerySimpleDownload_Util_Image(ServiceUtil::getManager());
    
            $objectType = 'download';
            $templateParameters[$objectType . 'ThumbPresetFileUpload'] = $imageHelper->getPreset($objectType, 'fileUpload', $context, $args);
            if (in_array($args['action'], array('display', 'view'))) {
                // use separate preset for images in related items
                $templateParameters['relationThumbPreset'] = $imageHelper->getCustomPreset('', '', 'VerySimpleDownload_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters = array();
        $parameters['catIdList'] = ModUtil::apiFunc('VerySimpleDownload', 'category', 'retrieveCategoriesFromRequest', array('ot' => 'download', 'source' => 'GET'));
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('VerySimpleDownload_Entity_Download', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('VerySimpleDownload_Entity_Download', 'tbl')
     ...
     *
     * @return array|VerySimpleDownload_Entity_Download retrieved data array or VerySimpleDownload_Entity_Download instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved VerySimpleDownload_Entity_Download instances
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('VerySimpleDownload', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'download', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if ($currentType == 'admin' && $currentModule == 'VerySimpleDownload') {
            return $qb;
        }
    
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved downloads only
            $onlineStates = array('approved');
            $showOnlyOwnEntries = (int) FormUtil::getPassedValue('own', ModUtil::getVar('VerySimpleDownload', 'showOnlyOwnEntries', 0), 'GETPOST');
            if ($showOnlyOwnEntries == 1) {
                // allow the owner to see his deferred downloads
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.downloadTitle LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.downloadDescription LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numDownloads';
        if ($useJoins === true) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('VerySimpleDownload_Entity_Download', 'tbl');
     ...
     * @param int    $excludeId  Id of downloads to exclude (optional).
     *
