 ...
 */
class Simpledownload_ContentType_Base_ItemList extends Content_AbstractContentType
{
    /**
    ...
    public function getTitle()
    {
        ...
        return __('Simpledownload list view', $dom);
    }
    ...
    public function getDescription()
    {
        ...
        return __('Display list of Simpledownload objects.', $dom);
    }
    ...
    public function loadData(&$data)
    {
        ...
        if (in_array($this->objectType, $this->categorisableObjectTypes)) {
            $idFields = ModUtil::apiFunc('Simpledownload', 'selection', 'getIdFields', array('ot' => $this->objectType));
            $this->catRegistries = ModUtil::apiFunc('Simpledownload', 'category', 'getAllPropertiesWithMainCat', array('ot' => $this->objectType, 'arraykey' => $idFields[0]));
            $this->catProperties = ModUtil::apiFunc('Simpledownload', 'category', 'getAllProperties', array('ot' => $this->objectType));
        }
    
        if (!isset($data['catIds'])) {
            $primaryRegistry = ModUtil::apiFunc('Simpledownload', 'category', 'getPrimaryProperty', array('ot' => $this->objectType));
    ...
    public function display()
    {
        ...
        $entityClass = 'Simpledownload_Entity_' . ucwords($this->objectType);
        $serviceManager = ServiceUtil::getManager();
        $entityManager = $serviceManager->getService('doctrine.entitymanager');
        $repository = $entityManager->getRepository($entityClass);
    
        // ensure that the view does not look for templates in the Content module (#218)
        $this->view->toplevelmodule = 'Simpledownload';
    
        $this->view->setCaching(Zikula_View::CACHE_ENABLED);
        // set cache id
        $component = 'Simpledownload:' . ucwords($this->objectType) . ':';
        $instance = '::';
        ...
        if (in_array($this->objectType, $this->categorisableObjectTypes)) {
            if (is_array($this->catIds) && count($this->catIds) > 0) {
                $qb = ModUtil::apiFunc('Simpledownload', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => $this->objectType, 'catids' => $this->catIds));
            }
        }
