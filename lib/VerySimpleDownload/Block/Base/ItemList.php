 ...
 */
class VerySimpleDownload_Block_Base_ItemList extends Zikula_Controller_AbstractBlock
{
    /**
    ...
    {
        SecurityUtil::registerPermissionSchema('VerySimpleDownload:ItemListBlock:', 'Block title::');
    
        $this->categorisableObjectTypes = array('download');
    }
    ...
    public function info()
    {
        ...
        return array('module'          => 'VerySimpleDownload',
                     'text_type'       => $this->__('VerySimpleDownload list view'),
                     'text_type_long'  => $this->__('Display list of VerySimpleDownload objects.'),
                     'allow_multiple'  => true,
                     'form_content'    => false,
                     'form_refresh'    => false,
                     'show_preview'    => true,
    ...
    public function display($blockinfo)
    {
        ...
        if (!SecurityUtil::checkPermission('VerySimpleDownload:ItemListBlock:', "$blockinfo[title]::", ACCESS_OVERVIEW)) {
            return false;
        }
        ...
        if (!isset($vars['catIds'])) {
            $primaryRegistry = ModUtil::apiFunc('VerySimpleDownload', 'category', 'getPrimaryProperty', array('ot' => $vars['objectType']));
        ...
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            $properties = ModUtil::apiFunc('VerySimpleDownload', 'category', 'getAllProperties', array('ot' => $objectType));
        }
    
        // apply category filters
        if (in_array($objectType, $this->categorisableObjectTypes)) {
            if (is_array($vars['catIds']) && count($vars['catIds']) > 0) {
                $qb = ModUtil::apiFunc('VerySimpleDownload', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => $objectType, 'catids' => $vars['catIds']));
            }
        }
        ...
        if (empty($blockinfo['title'])) {
            $blockinfo['title'] = $this->__('VerySimpleDownload items');
        }
    ...
    protected function getSortParam($vars, $repository)
    {
        ...
        if ($vars['sorting'] == 'newest') {
            $idFields = ModUtil::apiFunc('VerySimpleDownload', 'selection', 'getIdFields', array('ot' => $vars['objectType']));
            if (count($idFields) == 1) {
    ...
    public function update($blockinfo)
    {
        ...
        $vars['objectType'] = $this->request->request->filter('objecttype', 'download', FILTER_SANITIZE_STRING);
        ...
        $vars['catIds'] = array($primaryRegistry => array());
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            $vars['catIds'] = ModUtil::apiFunc('VerySimpleDownload', 'category', 'retrieveCategoriesFromRequest', array('ot' => $vars['objectType']));
        }
