 ...
 */
class Simpledownload_Entity_Repository_Base_Download extends EntityRepository
{
    /**
     * @var string The default sorting field/expression.
     */
    protected $defaultSortingField = 'doctitel';
    ...
    public function getDescriptionFieldName()
   {
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $listHelper = new Simpledownload_Util_ListEntries(ServiceUtil::getManager());
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('download', 'workflowState');
            }
    
            // initialise Imagine preset instances
            $imageHelper = new Simpledownload_Util_Image(ServiceUtil::getManager());
    
            $objectType = 'download';
            $templateParameters[$objectType . 'ThumbPresetUploaddocument'] = $imageHelper->getPreset($objectType, 'uploaddocument', $context, $args);
            if (in_array($args['action'], array('display', 'view'))) {
                // use separate preset for images in related items
                $templateParameters['relationThumbPreset'] = $imageHelper->getCustomPreset('', '', 'Simpledownload_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters = array();
        $parameters['catIdList'] = ModUtil::apiFunc('Simpledownload', 'category', 'retrieveCategoriesFromRequest', array('ot' => 'download', 'source' => 'GET'));
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('Simpledownload_Entity_Download', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('Simpledownload_Entity_Download', 'tbl')
     ...
     *
     * @return array|Simpledownload_Entity_Download retrieved data array or Simpledownload_Entity_Download instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved Simpledownload_Entity_Download instances
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('Simpledownload', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'download', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if ($currentType == 'admin' && $currentModule == 'Simpledownload') {
            return $qb;
        }
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= 'tbl.doctitel LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.docdescription LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.uploaddocument = \'' . $fragment . '\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.doctitel LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('Simpledownload_Entity_Download', 'tbl');
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        ...
        $showOnlyOwnEntries = (int) FormUtil::getPassedValue('own', ModUtil::getVar('Simpledownload', 'showOnlyOwnEntries', 0), 'GETPOST');
