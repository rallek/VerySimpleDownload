...
{if $numItems}
{capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='Simpledownload' type='admin' func='display' ot='download' id=$items[0].id}{/capture}
    <id>{$uniqueID}</id>
    <updated>{$items[0].updatedDate|default:$smarty.now|dateformat:'%Y-%m-%dT%H:%M:%SZ'}</updated>
{/if}
    <link rel="alternate" type="text/html" hreflang="{lang}" href="{modurl modname='Simpledownload' type='admin' func='main' fqurl=1}" />
...
{foreach item='download' from=$items}
    <entry>
        ...
        <link rel="alternate" type="text/html" href="{modurl modname='Simpledownload' type='admin' func='display' ot='download' id=$download.id fqurl='1'}" />

        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$download.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='Simpledownload' type='admin' func='display' ot='download' id=$download.id}{/capture}
        <id>{$uniqueID}</id>
