 ...
 */
class Simpledownload_Form_Plugin_Base_ItemSelector extends Zikula_Form_Plugin_TextInput
{
    /**
    ...
    public function render(Zikula_Form_View $view)
    {
        ...
        if ($firstTime) {
            ...
            PageUtil::addVar('javascript', 'modules/Simpledownload/javascript/Simpledownload_finder.js');
            PageUtil::addVar('stylesheet', ThemeUtil::getModuleStylesheet('Simpledownload'));
        }
        $firstTime = false;

        if (!SecurityUtil::checkPermission('Simpledownload:' . ucwords($this->objectType) . ':', '::', ACCESS_COMMENT)) {
            return false;
        }
        ...
        if (in_array($this->objectType, $categorisableObjectTypes)) {
            ...
            $catIds = ModUtil::apiFunc('Simpledownload', 'category', 'retrieveCategoriesFromRequest', array('ot' => $this->objectType));
        }
        ...
        $properties = null;
        if (in_array($this->objectType, $categorisableObjectTypes)) {
            $properties = ModUtil::apiFunc('Simpledownload', 'category', 'getAllProperties', array('ot' => $this->objectType));
        }
    ...
    {
        parent::decode($view);
        $this->objectType = FormUtil::getPassedValue('Simpledownload_objecttype', 'download', 'POST');
        $this->selectedItemId = $this->text;
    }
}
