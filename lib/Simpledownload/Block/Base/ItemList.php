 ...
 */
class Simpledownload_Block_Base_ItemList extends Zikula_Controller_AbstractBlock
{
    /**
    ...
    {
        SecurityUtil::registerPermissionSchema('Simpledownload:ItemListBlock:', 'Block title::');
    ...
    public function info()
    {
        ...
        if (!ModUtil::available('Simpledownload')) {
            $requirementMessage .= $this->__('Notice: This block will not be displayed until you activate the Simpledownload module.');
        }
    
        return array('module'          => 'Simpledownload',
                     'text_type'       => $this->__('Simpledownload list view'),
                     'text_type_long'  => $this->__('Display list of Simpledownload objects.'),
                     'allow_multiple'  => true,
                     'form_content'    => false,
                     'form_refresh'    => false,
                     'show_preview'    => true,
    ...
    public function display($blockinfo)
    {
        ...
        if (!SecurityUtil::checkPermission('Simpledownload:ItemListBlock:', "$blockinfo[title]::", ACCESS_OVERVIEW)) {
            return false;
        }
        ...
        if (!isset($vars['catIds'])) {
            $primaryRegistry = ModUtil::apiFunc('Simpledownload', 'category', 'getPrimaryProperty', array('ot' => $vars['objectType']));
        ...
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            $properties = ModUtil::apiFunc('Simpledownload', 'category', 'getAllProperties', array('ot' => $objectType));
        }
    
        // apply category filters
        if (in_array($objectType, $this->categorisableObjectTypes)) {
            if (is_array($vars['catIds']) && count($vars['catIds']) > 0) {
                $qb = ModUtil::apiFunc('Simpledownload', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => $objectType, 'catids' => $vars['catIds']));
            }
        }
        ...
        if (empty($blockinfo['title'])) {
            $blockinfo['title'] = $this->__('Simpledownload items');
        }
    ...
    protected function getSortParam($vars, $repository)
    {
        ...
        if ($vars['sorting'] == 'newest') {
            $idFields = ModUtil::apiFunc('Simpledownload', 'selection', 'getIdFields', array('ot' => $vars['objectType']));
            if (count($idFields) == 1) {
    ...
    public function update($blockinfo)
    {
        ...
        $vars['catIds'] = array($primaryRegistry => array());
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            $vars['catIds'] = ModUtil::apiFunc('Simpledownload', 'category', 'retrieveCategoriesFromRequest', array('ot' => $vars['objectType']));
        }
