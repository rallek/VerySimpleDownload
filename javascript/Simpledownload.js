...
var simdownContextMenu;

simdownContextMenu = Class.create(Zikula.UI.ContextMenu, {
 ...
 */
function simdownInitItemActions(objectType, func, containerId)
{
    ...
    // attach context menu
    contextMenu = new simdownContextMenu(triggerId, { leftClick: true, animation: false });
...
function simdownCapitaliseFirstLetter(string)
{
 ...
 */
function simdownSubmitQuickNavForm(objectType)
{
    $('simpledownload' + simdownCapitaliseFirstLetter(objectType) + 'QuickNavForm').submit();
}

/**
 * Initialise the quick navigation panel in list views.
 */
function simdownInitQuickNavigation(objectType, controller)
{
    if ($('simpledownload' + simdownCapitaliseFirstLetter(objectType) + 'QuickNavForm') == undefined) {
        return;
    }

    if ($('catid') != undefined) {
        $('catid').observe('change', function () { simdownSubmitQuickNavForm(objectType); });
    }
    if ($('sortby') != undefined) {
        $('sortby').observe('change', function () { simdownSubmitQuickNavForm(objectType); });
    }
    if ($('sortdir') != undefined) {
        $('sortdir').observe('change', function () { simdownSubmitQuickNavForm(objectType); });
    }
    if ($('num') != undefined) {
        $('num').observe('change', function () { simdownSubmitQuickNavForm(objectType); });
    }

    switch (objectType) {
    case 'download':
        if ($('workflowState') != undefined) {
            $('workflowState').observe('change', function () { simdownSubmitQuickNavForm(objectType); });
        }
        break;
    default:
        break;
